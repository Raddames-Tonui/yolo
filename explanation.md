...
The `docker-compose.yml` file configures the services with a custom bridge network (`yolo-network`) to ensure seamless communication between containers.
...
The `docker-compose.yml` file defines a volume for MongoDB to persist data across container restarts or deletions. The MongoDB data is stored in a named volume `mongodata`, which ensures that the data remains intact even when the container is stopped or removed.
